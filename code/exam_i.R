...
g_scat <- ggplot(iris, aes(x = Sepal.Width, y = Sepal.Length)) +
  geom_point()
...
g_scat_col <- ggplot(iris, aes(x = Petal.Width, y = Petal.Length, color = Species)) +
  geom_point()
# 13. Histogram of `Petal.Width` with `binwidth = 0.5`
# Assign to: `g_hist`
# Create a histogram of `Petal.Width` with `binwidth` set to 0.5.
g_hist <- ggplot(iris, aes(x = Petal.Width)) +
...
g_bplot1 <- ggplot(PlantGrowth, aes(x = group, y = weight, fill = group)) +
  geom_boxplot()
...
g_bplot2 <- ggplot(PlantGrowth, aes(x = group, y = weight)) +
  geom_boxplot() +
  geom_point()